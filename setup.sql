...
-- Invite Codes RLS Policies (idempotent)
...
-- Backward-compatible schema upgrades (safe on re-run)
...
-- Challenges RLS Policies (idempotent)
DROP POLICY IF EXISTS "Anyone can view published challenges" ON public.challenges;
...
-- User Progress RLS Policies (idempotent)
...
-- Challenge Submissions RLS Policies (idempotent)
...
DROP POLICY IF EXISTS "Users can create own submissions" ON public.challenge_submissions;
CREATE POLICY "Users can create own submissions"
ON public.challenge_submissions
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all submissions" ON public.challenge_submissions;
...
-- Challenge Bookmarks RLS Policies (idempotent)
DROP POLICY IF EXISTS "Users can view own bookmarks" ON public.challenge_bookmarks;
CREATE POLICY "Users can view own bookmarks"
ON public.challenge_bookmarks
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create own bookmarks" ON public.challenge_bookmarks;
CREATE POLICY "Users can create own bookmarks"
ON public.challenge_bookmarks
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own bookmarks" ON public.challenge_bookmarks;
...
-- Pathways RLS Policies (idempotent)
DROP POLICY IF EXISTS "Anyone can view pathways" ON public.pathways;
...
DROP POLICY IF EXISTS "Admins can manage pathways" ON public.pathways;
...
-- Pathway Challenges RLS Policies (idempotent)
DROP POLICY IF EXISTS "Anyone can view pathway challenges" ON public.pathway_challenges;
...
DROP POLICY IF EXISTS "Admins can manage pathway challenges" ON public.pathway_challenges;
...
-- User Pathway Progress RLS Policies (idempotent)
DROP POLICY IF EXISTS "Users can view own pathway progress" ON public.user_pathway_progress;
CREATE POLICY "Users can view own pathway progress"
ON public.user_pathway_progress
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all pathway progress" ON public.user_pathway_progress;
...
-- ============================================
-- Insert sample pathways (safe to re-run)
INSERT INTO public.pathways (name, description, category, total_challenges, is_active)
SELECT
    seed.name,
    seed.description,
    seed.category,
    seed.total_challenges,
    seed.is_active
FROM (
    VALUES
...
) AS seed(name, description, category, total_challenges, is_active)
WHERE NOT EXISTS (
    SELECT 1 FROM public.pathways p WHERE p.name = seed.name
);

-- Insert additional challenge catalog (20+ - idempotent by title)
...
-- Insert sample challenges (idempotent by title)
INSERT INTO public.challenges (
...
FROM (
    VALUES
        ('The Infinite Voyager',
         ...
         '[{"input": "calculate_journey(150, 60)", "expected": "Travel time: 2.5 hours"}, {"input": "calculate_journey(300, 50)", "expected": "Travel time: 6.0 hours"}]',
        ...
        ('List Comprehension Master',
         ...
         '[{"input": "[x**2 for x in range(1, 6)]", "expected": "[1, 4, 9, 16, 25]"}]',
        ...
        ('Dictionary Detective',
         ...
         '[{"input": "get_nested_value({\"a\": {\"b\": 1}}, [\"a\", \"b\"])", "expected": "1"}, {"input": "get_nested_value({}, [\"x\"])", "expected": "None"}]',
         'Alex',
         30,
         20,
         TRUE),
        ...
        ('Array Mastery',
         ...
         '[{"input": "rotate_array([1,2,3,4,5], 2)", "expected": "[4, 5, 1, 2, 3]"}, {"input": "rotate_array([1,2], 3)", "expected": "[2, 1]"}]',
         '[4, 5, 1, 2, 3]',
         45,
         25,
         TRUE)
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.challenges c WHERE c.title = seed.title
);
